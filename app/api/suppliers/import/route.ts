...
function parseCsv(content: string): ImportRow[] {
  ...
  for (let lineIndex = 1; lineIndex < lines.length; lineIndex += 1) {
    ...
    const row: ImportRow = {
      supplierCode: getCell(cells, ["supplierCode", "codigoProveedor"]),
    ...
    if (!row.name && !row.supplierCode) continue;
    rows.push(row);
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      try {
        const supplierCodeRaw = String(row.supplierCode ?? "").trim().toUpperCase();
        const isEdit = supplierCodeRaw.length > 0;
        ...
        if (!isEdit) {
          ...
          const [duplicate] = await db
            ...
            .from(suppliers)
            .where(eq(suppliers.identification, identification))
            .limit(1);

          if (duplicate) {
            throw new Error(
              `Ya existe un proveedor con identificación: ${identification}`
            );
          }
        ...
        const [existing] = await db
          .select()
          .from(suppliers)
          .where(eq(suppliers.supplierCode, supplierCodeRaw))
          .limit(1);

        if (!existing) {
          throw new Error(`supplierCode no existe para edición: ${supplierCodeRaw}`);
        }
        ...
        if (Object.keys(patch).length === 0) {
          throw new Error(`Fila sin cambios para edición (${supplierCodeRaw})`);
        }

        await db.update(suppliers).set(patch).where(eq(suppliers.id, existing.id));
    ...
    return new Response("No se pudo importar proveedores desde CSV", { status: 500 });
  }
}
