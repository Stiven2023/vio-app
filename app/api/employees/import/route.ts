...
function parseCsv(content: string): ImportRow[] {
  ...
  for (let lineIndex = 1; lineIndex < lines.length; lineIndex += 1) {
    ...
    const row: ImportRow = {
      employeeCode: getCell(cells, ["employeeCode", "codigoEmpleado"]),
      ...
      contractType: getCell(cells, ["contractType", "tipoContrato"]),
      isActive: getCell(cells, ["isActive", "activo", "estado"]),
    };

    if (!row.name && !row.employeeCode) continue;
    rows.push(row);
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      try {
        const employeeCodeRaw = String(row.employeeCode ?? "").trim().toUpperCase();
        const isEdit = employeeCodeRaw.length > 0;
        ...
        if (!isEdit) {
          ...
          if (duplicate) {
            throw new Error(
              `Ya existe un empleado con identificación: ${identification}`
            );
          }
        ...
        const [existing] = await db
          .select()
          .from(employees)
          .where(eq(employees.employeeCode, employeeCodeRaw))
          .limit(1);

        if (!existing) {
          throw new Error(`employeeCode no existe para edición: ${employeeCodeRaw}`);
        }
        ...
        if (Object.keys(patch).length === 0) {
          throw new Error(`Fila sin cambios para edición (${employeeCodeRaw})`);
        }

        await db.update(employees).set(patch).where(eq(employees.id, existing.id));
    ...
    return new Response("No se pudo importar empleados desde CSV", { status: 500 });
  }
}
