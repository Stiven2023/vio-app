...
/** PUT /api/confectionists/payment-requests/[id]/verify */
export async function PUT(
  request: Request,
  { params }: { params: Promise<{ id: string }> },
) {
  const limited = rateLimit(request, {
    key: "confectionists:payment-requests:verify",
    limit: 60,
    windowMs: 60_000,
  });
  ...
  const forbidden = await requirePermission(request, "VERIFICAR_PAGO_CONFECCION");
  ...
  try {
    ...
    if (result.kind === "invalid-status") {
      return new Response(`Estado inválido para verificar: ${result.status}`, { status: 422 });
    }
