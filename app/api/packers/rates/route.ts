...
import { packerRates } from "@/src/db/erp/schema";
...
export async function GET(request: Request) {
  ...
  const forbidden = await requirePermission(request, "VER_EMPACADORES");
  ...
  try {
    ...
    if (activeOnly) conditions.push(eq(packerRates.isActive, true));
    if (garmentType) conditions.push(eq(packerRates.garmentType, garmentType));

    const rows = await db
      .select()
      .from(packerRates)
      .where(conditions.length > 0 ? and(...conditions) : undefined)
      .orderBy(desc(packerRates.validFrom));
