...
const CONTRACT_LABELS: Record<ContractTypeKey, string> = {
  FIXED_TERM: "TÃ©rmino fijo",
  INDEFINITE_TERM: "TÃ©rmino indefinido",
  WORK_CONTRACT: "Obra y labor",
  SERVICE_CONTRACT: "PrestaciÃ³n de servicios",
};
...
export function PilaTab({ canGenerate }: { canGenerate: boolean }) {
  ...
  async function openPreview() {
    if (!period) {
      toast.error("Selecciona un perÃ­odo");
  ...
  async function confirmGenerate() {
    ...
    try {
      ...
      setPreview(response);
      toast.success("PILA generada y perÃ­odo bloqueado");
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-sm text-default-500">
          LiquidaciÃ³n de aportes PILA con base en provisiones de nÃ³mina.
        </div>
        {canGenerate ? (
          <Button
            color="primary"
            isDisabled={
              Boolean(pilaData?.generation.isGenerated) || previewLoading
            }
            startContent={<BsPlus />}
            onPress={openPreview}
          >
      ...
      {pilaData?.generation.isGenerated ? (
        <div className="rounded-medium border border-success-200 bg-success-50 p-3 text-sm text-success-800">
          PerÃ­odo bloqueado por generaciÃ³n PILA. Generado el{" "}
