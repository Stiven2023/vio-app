...
import type { Supplier, LegalStatusRecord } from "../../_lib/types";
import { updateSupplierLegalStatus, getSupplierLegalStatusHistory } from "../../_lib/supplier-legal-status";
...
export function SupplierLegalStatusModal({
  supplier,
  isOpen,
  onOpenChange,
}: {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      setLoading(true);
      try {
        const data = await getSupplierLegalStatusHistory(supplier.id);
        setHistory(data || []);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateSupplierLegalStatus(supplier.id, {
        status,
    ...
    <Modal isOpen={isOpen} size="3xl" onOpenChange={onOpenChange}>
      <ModalContent>
        <ModalHeader className="flex gap-2">
          ...
          <span>Estado Jurídico - {supplier.name}</span>
        </ModalHeader>
        <Divider />
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-warning-900">
                      Proveedor será desactivado automáticamente
                    </p>
                    <p className="text-xs text-warning-800 mt-1">
                      {status === "EN_REVISION"
                        ? "El proveedor no podrá operar mientras su estado esté en revisión."
                        : "El proveedor no podrá operar mientras esté bloqueado."}
                    </p>
                  </div>
                </div>
              </CardBody>
