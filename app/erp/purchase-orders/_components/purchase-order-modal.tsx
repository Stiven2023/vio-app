          ...
          <div className="overflow-hidden rounded-3xl border border-warning-200 bg-content1">
            <div className="border-b border-warning-100 bg-gradient-to-r from-amber-100 via-orange-50 to-white px-5 py-4">
              ...
              <div className="mt-2 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <h2 className="text-xl font-semibold text-default-900">Orden de compra</h2>
                  <p className="text-sm text-default-600">
                    Código automático al guardar. Todos los valores se registran en COP.
                  </p>
                </div>
                ...
                <div className="space-y-4 rounded-2xl border border-default-200 bg-default-50/70 p-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-default-500">
                      Selecciona el proveedor y completa el detalle de reposición.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <Select
                      isDisabled={submitting || loadingOptions}
                      label="Banco"
                      selectedKeys={bankName ? new Set([bankName]) : new Set([])}
                      onSelectionChange={(keys) => {
                        const first = Array.from(keys)[0];
                  ...
                  <div className="grid gap-3 rounded-2xl border border-default-200 bg-white p-4 sm:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-wide text-default-500">Contacto</div>
                      <div className="mt-1 text-sm font-medium text-default-800">
                        {selectedSupplier?.contactName ?? "Sin definir"}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-wide text-default-500">Correo</div>
                      <div className="mt-1 text-sm font-medium text-default-800">
                        {selectedSupplier?.email ?? "Sin definir"}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-wide text-default-500">Identificación</div>
                      <div className="mt-1 text-sm font-medium text-default-800">
                        {selectedSupplier?.identification ?? "Sin definir"}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-wide text-default-500">Teléfono</div>
                      <div className="mt-1 text-sm font-medium text-default-800">
                        {selectedSupplier?.fullMobile ?? selectedSupplier?.mobile ?? "Sin definir"}
                      </div>
                    </div>
                    <div className="sm:col-span-2">
                      <div className="text-xs uppercase tracking-wide text-default-500">Dirección</div>
                      <div className="mt-1 text-sm font-medium text-default-800">
                        {selectedSupplier
                ...
                <div className="space-y-3 rounded-2xl border border-warning-200 bg-warning-50/60 p-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-warning-900/70">
                      Vista rápida para validar cantidades, costos y total antes de emitir la orden.
                    </p>
                  </div>
                  ...
                  <div className="space-y-2 rounded-2xl bg-white p-4 shadow-sm">
                    ...
                    <div className="flex items-center justify-between text-sm text-default-600">
                      <span>Banco</span>
                      <span className="max-w-[180px] truncate text-right">{bankName || "-"}</span>
                    </div>
              ...
              <div className="space-y-3">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-default-500">
                      Define cantidad y valor unitario por cada item del inventario.
                    </p>
                  </div>
                  <Button variant="flat" isDisabled={submitting} onPress={addEmptyRow}>
                    Agregar línea
                  </Button>
                </div>
                  ...
                  {itemRows.map((row, index) => (
                    ...
                    >
                          ...
                          {(item) => (
                            <SelectItem key={item.id} textValue={item.name}>
                              <div className="flex flex-col">
                                <span>{item.name}</span>
                                <span className="text-xs text-default-500">
                                  {(item.itemCode ?? "Sin código") + " · " + (item.unit ?? "und")}
                                </span>
                              </div>
                            </SelectItem>
                          )}
                      ...
                      <div className="mt-3 grid gap-3 text-sm text-default-500 sm:grid-cols-3">
                        <div>
                          Código: <span className="font-medium text-default-700">{row.source?.itemCode ?? "-"}</span>
                        </div>
                        <div>
                          Unidad: <span className="font-medium text-default-700">{row.source?.unit ?? "-"}</span>
                        </div>
                        <div>
                          Precio sugerido: <span className="font-medium text-default-700">{formatMoney(parseNumeric(String(row.source?.price ?? "0")))}</span>
                        </div>
                      </div>
                    </div>
                  ))}
